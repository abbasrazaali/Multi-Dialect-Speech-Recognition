...
from util.utils import logging, checkPathExists, audio_formating
...
from util.text import sparse_tensor_value_to_texts, wer, levenshtein, Alphabet, normalize
...
# from tensorflow.python.ops.rnn_cell_impl import MultiRNNCell, GRUCell # DropoutWrapper, LSTMCell
from tensorflow.nn.rnn_cell import RNNCell, GRUCell, BasicLSTMCell, MultiRNNCell, DropoutWrapper

language = 'arabic'
# language = 'english'

activation_functions = {'sigmoid': tf.sigmoid, 'tanh': tf.tanh, 'relu': tf.nn.relu, 'relu6': tf.nn.relu6,
                        'elu': tf.nn.elu, 'softplus': tf.nn.softplus, 'softsign': tf.nn.softsign}

rnn_function = {'rnn': RNNCell, 'gru': GRUCell, 'lstm': BasicLSTMCell}
...
def create_flags():
    ...
    tf.app.flags.DEFINE_string  ('model',             'deepspeech_2',         'model to use')

    tf.app.flags.DEFINE_string  ('train_files',      data_path + 'features/' + language + '/' + FLAGS.model + '/train.csv',             'comma separated list of files specifying the dataset used for training. multiple files will get merged')
    tf.app.flags.DEFINE_string  ('dev_files',        data_path + 'features/' + language + '/' + FLAGS.model + '/dev.csv',               'comma separated list of files specifying the dataset used for validation. multiple files will get merged')
    tf.app.flags.DEFINE_string  ('test_files',       data_path + 'features/' + language + '/' + FLAGS.model + '/test.csv',              'comma separated list of files specifying the dataset used for testing. multiple files will get merged')
    ...
    # tf.app.flags.DEFINE_string  ('train_cached_features_path',      data_path + 'features/' + language + '/' + FLAGS.model + '/train_cached.csv',          'comma separated list of files specifying the dataset used for training. multiple files will get merged')
    # tf.app.flags.DEFINE_string  ('dev_cached_features_path',        data_path + 'features/' + language + '/' + FLAGS.model + '/dev_cached.csv',          'comma separated list of files specifying the dataset used for validation. multiple files will get merged')
    # tf.app.flags.DEFINE_string  ('test_cached_features_path',       data_path + 'features/' + language + '/' + FLAGS.model + '/test_cached.csv',          'comma separated list of files specifying the dataset used for testing. multiple files will get merged')
    ...
    if('english' in language):
        ...
        tf.app.flags.DEFINE_integer('coord_port',       2501,     'coordination server port')
    ...
    tf.app.flags.DEFINE_integer ('epoch',            180,         'target epoch to train - if negative, the absolute number of additional epochs will be trained')

    tf.app.flags.DEFINE_boolean ('use_warpctc',      False,       'whether to use GPU bound Warp-CTC')

    tf.app.flags.DEFINE_float   ('dropout_rate',     0.2,         'dropout rate for feedforward layers')
    ...
    # deepspeech-2
    tf.app.flags.DEFINE_integer('n_filters',         32,          'number of filters for deepspeech-2 convolutional layers')
    # if('english' in language):
    tf.app.flags.DEFINE_integer('n_cnn_layers',     2,            'number of cnn layers')
    tf.app.flags.DEFINE_integer('n_rnn_layers',     3,            'number of rnn layers')

    tf.app.flags.DEFINE_integer('n_hidden',         1024,         'layer width to use when initialising layers')

    tf.app.flags.DEFINE_string('rnn_cell',          'lstm',        'rnn, gru, lstm')
    ...
    # else:
    #     tf.app.flags.DEFINE_integer('n_cnn_layers',   2,           'number of cnn layers')
    #     tf.app.flags.DEFINE_integer('n_rnn_layers',   5,           'number of rnn layers')
    #
    #     tf.app.flags.DEFINE_integer('n_hidden', 1510, 'layer width to use when initialising layers')
    #
    #     tf.app.flags.DEFINE_string('rnn_cell', 'lstm', 'rnn, gru, lstm')
    #
    #     # Batch sizes
    #     tf.app.flags.DEFINE_integer('train_batch_size', 8, 'number of elements in a training batch')
    #     tf.app.flags.DEFINE_integer('dev_batch_size', 8, 'number of elements in a validation batch')
    #     tf.app.flags.DEFINE_integer('test_batch_size', 8, 'number of elements in a test batch')

    # tf.app.flags.DEFINE_integer('n_hidden',         1510,          'layer width to use when initialising layers')


    tf.app.flags.DEFINE_string('activation',        'relu6',       'activation function')
    tf.app.flags.DEFINE_boolean('batch_norm',       True,          'batch normalization')
    tf.app.flags.DEFINE_boolean('fused',            True,          'fused layers')

    # Adam optimizer (http://arxiv.org/abs/1412.6980) parameters

    tf.app.flags.DEFINE_float   ('beta1',            0.90,         'beta 1 parameter of Adam optimizer')
    ...
    # batch normalization
    tf.app.flags.DEFINE_float('batch_norm_momentum', 0.99,        'batch normalization momentum')
    tf.app.flags.DEFINE_float('batch_norm_epsilon',  1e-3,        'batch normalization epsilon')
    ...
    tf.app.flags.DEFINE_integer ('validation_step',  100,           'number of epochs we cycle through before validating the model - a detailed progress report is dependent on "--display_step" - 0 means no validation steps')

    # Checkpointing

    tf.app.flags.DEFINE_string  ('checkpoint_dir',   'model/' + language + '/',          'directory in which checkpoints are stored - defaults to directory "deepspeech/checkpoints" within user\'s data home specified by the XDG Base Directory Specification')
    tf.app.flags.DEFINE_string  ('log_dir', 'log/' + language + '/', 'directory in which checkpoints are stored - defaults to directory "deepspeech/checkpoints" within user\'s data home specified by the XDG Base Directory Specification')
    tf.app.flags.DEFINE_integer ('checkpoint_secs',  3600,         'checkpoint saving interval in seconds')
    tf.app.flags.DEFINE_integer ('max_to_keep',      1,            'number of checkpoint files to keep - default value is 5')

    # Exporting

    tf.app.flags.DEFINE_string  ('export_dir',       'model/' + language + '/',          'directory in which exported models are stored - if omitted, the model won\'t get exported')
    ...
    tf.app.flags.DEFINE_boolean ('show_progressbar', False,        'Show progress for training, validation and testing processes. Log level should be > 0.')

    tf.app.flags.DEFINE_string  ('wer_log_pattern',  'log/' + language + '/wer/',          'pattern for machine readable global logging of WER progress; has to contain %%s, %%s and %%f for the set name, the date and the float respectively; example: "GLOBAL LOG: logwer(\'12ade231\', %%s, %%s, %%f)" would result in some entry like "GLOBAL LOG: logwer(\'12ade231\', \'train\', \'2017-05-18T03:09:48-0700\', 0.05)"; if omitted (default), there will be no logging')
    ...
    tf.app.flags.DEFINE_string  ('summary_dir',      'model/' + language + '/log',          'target directory for TensorBoard summaries - defaults to directory "deepspeech/summaries" within user\'s data home specified by the XDG Base Directory Specification')
    ...
    tf.app.flags.DEFINE_boolean('lm_decoding',      True,          'language_model')
    ...
    tf.app.flags.DEFINE_string  ('decoder_library_path', 'native_client/lib/libctc_decoder_with_kenlm.so', 'path to the libctc_decoder_with_kenlm.so library containing the decoder implementation.')
    tf.app.flags.DEFINE_string  ('alphabet_config_path', 'language_model/' + language + '/alphabet.txt', 'path to the configuration file specifying the alphabet used by the network. See the comment in data/alphabet.txt for a description of the format.')
    tf.app.flags.DEFINE_string  ('lm_binary_path',       'language_model/' + language + '/lm.binary', 'path to the language model binary file created with KenLM')
    tf.app.flags.DEFINE_string  ('lm_trie_path',         'language_model/' + language + '/trie', 'path to the language model trie file created with native_client/generate_trie')
    tf.app.flags.DEFINE_integer ('beam_width',        512,       'beam width used in the CTC decoder when building candidate transcriptions')
    tf.app.flags.DEFINE_float   ('lm_weight',         0.75,       'the alpha hyperparameter of the CTC decoder. Language Model weight.')
    tf.app.flags.DEFINE_float   ('valid_word_count_weight', 1.85, 'valid word insertion weight. This is used to lessen the word insertion penalty when the inserted word is part of the vocabulary.')
    ...
    # tf.app.flags.DEFINE_string('one_shot_infer', 'infer/' + language + '/', 'one-shot inference mode: specify a wav file and the script will load the checkpoint and perform inference on it. Disables training, testing and exporting.')
    ...
    # tf.app.flags.DEFINE_string ('initialize_from_frozen_model', 'model/' + language + '/' + language + '_asr_graph.pb', 'path to frozen model to initialize from. This behaves like a checkpoint, loading the weights from the frozen model and starting training with those weights. The optimizer parameters aren\'t restored, so remember to adjust the learning rate.')
...
# deepspeech_1
def deepspeech_1(batch_x, seq_length, dropout, reuse=False, batch_size=None, n_steps=-1, previous_state=None, mode = 'train'):
    r'''
    ...
    return layer_6, seq_length, layers

# deepspeech-2
def deepspeech_2(batch_x, seq_length, dropout = 0.0, reuse=False, batch_size=None, n_steps=-1, previous_state=None, mode = 'train'):
    layers = {}

    batch_size = tf.shape(batch_x)[0]       # Input shape: [batch, megnitude<time, frequency>]

    input_batch = tf.expand_dims(batch_x, dim=-1)

    data_format = ('channels_first' if tf.test.is_built_with_cuda() else 'channels_last')

    if data_format == 'channels_first':
        input_batch = tf.transpose(input_batch, [0, 3, 1, 2])

    # convolutional layers
    # padding = (20, 5)
        # input_batch = tf.pad(input_batch, [[0, 0], [padding[0], padding[0]], [padding[1], padding[1]], [0, 0]])
    net = tf.layers.conv2d(inputs=input_batch, filters=FLAGS.n_filters, kernel_size=[41, 11], strides=[2, 2], padding='valid', use_bias=False,
                           kernel_initializer=tf.variance_scaling_initializer(), activation=activation_functions[FLAGS.activation], data_format=data_format, trainable=(mode == 'train'), name="conv_layer1")
    if (FLAGS.batch_norm):
        net = tf.layers.batch_normalization(net, axis=1 if data_format == 'channels_first' else -1, momentum=FLAGS.batch_norm_momentum, epsilon=FLAGS.batch_norm_epsilon, fused=FLAGS.fused, training=(mode == 'train'), name="layer1_batch_norm")
    net = _relux(net, capping=FLAGS.relu_clip)
    layers['cov_layer_1'] = net

    # padding = (10, 5)
    # net = tf.pad(net, [[0, 0], [padding[0], padding[0]], [padding[1], padding[1]], [0, 0]])
    net = tf.layers.conv2d(inputs=net, filters=FLAGS.n_filters, kernel_size=[21, 11], strides=[2, 1], padding='valid', use_bias=False,
                           kernel_initializer=tf.variance_scaling_initializer(), activation=activation_functions[FLAGS.activation], data_format=data_format, trainable=(mode == 'train'), name="conv_layer2")
    if (FLAGS.batch_norm):
        net = tf.layers.batch_normalization(net, axis=1 if data_format == 'channels_first' else -1, momentum=FLAGS.batch_norm_momentum, epsilon=FLAGS.batch_norm_epsilon, fused=FLAGS.fused, training=(mode == 'train'), name="layer2_batch_norm")
    net = _relux(net, capping=FLAGS.relu_clip)
    layers['cov_layer_2'] = net

    if(FLAGS.n_cnn_layers > 2):
        # padding = (10, 5)
        # net = tf.pad(net, [[0, 0], [padding[0], padding[0]], [padding[1], padding[1]], [0, 0]])
        net = tf.layers.conv2d(inputs=net, filters=FLAGS.n_filters * 3, kernel_size=[21, 11], strides=[2, 1], padding='valid', use_bias=False,
                               kernel_initializer=tf.variance_scaling_initializer(), activation=activation_functions[FLAGS.activation], data_format=data_format, trainable=(mode == 'train'), name="conv_layer3")
        if (FLAGS.batch_norm):
            net = tf.layers.batch_normalization(net, axis=1 if data_format == 'channels_first' else -1, momentum=FLAGS.batch_norm_momentum, epsilon=FLAGS.batch_norm_epsilon, fused=FLAGS.fused, training=(mode == 'train'), name="layer3_batch_norm")
        net = _relux(net, capping=FLAGS.relu_clip)
        layers['cov_layer_3'] = net

    if data_format == 'channels_first':
        net = tf.transpose(net, [0, 2, 3, 1])

    # reshape to [B, T, FxC]
    f = net.get_shape().as_list()[2]
    c = net.get_shape().as_list()[3]
    net = tf.reshape(net, [batch_size, -1, f * c])  # reshape conv output to fit rnn input: N, T, F * C
    net = tf.transpose(net, perm=[1, 0, 2])         # permute into time major order for rnn: T, N, F * C

    seq_len_of_data = tf.cast(seq_length, dtype=tf.float64)
    seq_len_of_data = tf.ceil(tf.div(tf.subtract(seq_len_of_data, 40), 2.0))
    if (FLAGS.n_cnn_layers > 2):
        seq_len_of_data = tf.ceil(tf.div(tf.subtract(seq_len_of_data, 20), 2.0))
    seq_len_of_data = tf.cast(tf.ceil(tf.div(tf.subtract(seq_len_of_data, 20), 2.0)), tf.int32)

    fw_lstm_cells = []
    bw_lstm_cells = []

    for index in range(FLAGS.n_rnn_layers):
        with tf.variable_scope('fw_cell_{}'.format(str(index))):
            # Forward direction cell:
            # lstm_fw_cell = rnn_function[FLAGS.rnn_cell](n_cell_dim, forget_bias=1.0, state_is_tuple=True, reuse=tf.get_variable_scope().reuse)        # rnn
            lstm_fw_cell = rnn_function[FLAGS.rnn_cell](n_cell_dim, reuse=tf.get_variable_scope().reuse)
            lstm_fw_cell = DropoutWrapper(lstm_fw_cell, input_keep_prob=(1.0 - dropout), output_keep_prob=(1.0 - dropout), seed=FLAGS.random_seed)

            fw_lstm_cells.append(lstm_fw_cell)

            layers['fw_cell_{}'.format(str(index))] = fw_lstm_cells
        with tf.variable_scope('bw_cell_{}'.format(str(index))):
            # Backward direction cell:
            # lstm_bw_cell = rnn_function[FLAGS.rnn_cell](n_cell_dim, forget_bias=1.0, state_is_tuple=True, reuse=tf.get_variable_scope().reuse)    # rnn
            lstm_bw_cell = rnn_function[FLAGS.rnn_cell](n_cell_dim, reuse=tf.get_variable_scope().reuse)
            lstm_bw_cell = DropoutWrapper(lstm_bw_cell, input_keep_prob=(1.0 - dropout), output_keep_prob=(1.0 - dropout), seed=FLAGS.random_seed)

            bw_lstm_cells.append(lstm_bw_cell)
            layers['bw_cell_{}'.format(str(index))] = bw_lstm_cells

    fw_cell = MultiRNNCell(cells=fw_lstm_cells)
    bw_cell = MultiRNNCell(cells=bw_lstm_cells)

    # feed `cov_layer` into the LSTM BRNN cell and obtain the LSTM BRNN output.
    outputs, output_states = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell, cell_bw=bw_cell, inputs=net, dtype=net.dtype, time_major=True, sequence_length=seq_len_of_data)

    # Reshape outputs from two tensors each of shape [n_steps, batch_size, n_cell_dim] to a single tensor of shape [n_steps*batch_size, 2*n_cell_dim]
    net = tf.concat(outputs, 2)

    layers['rnn_output'] = outputs
    layers['rnn_output_state'] = output_states

    logits = tf.layers.dense(inputs=net, units=n_hidden_6, kernel_initializer=tf_contrib.layers.variance_scaling_initializer(), kernel_regularizer=tf_contrib.layers.l2_regularizer(0.0005), name="logits")        # activation=tf.nn.relu
    layers['logits'] = logits

    return logits, layers, seq_len_of_data


def _variable_on_cpu(name, shape, initializer=None, trainable=True):
    with tf.device('/cpu'):
        var = tf.get_variable(name, shape, initializer=initializer, dtype=tf.float32, trainable=trainable)
    return var

def _relux(x, capping=None):
    x = tf.nn.relu(x)
    return x if capping is None else tf.minimum(x, capping)
...
def decode_with_lm(inputs, sequence_length, beam_width=100,
  ...
  decoded_ixs, decoded_vals, decoded_shapes, log_probabilities = (
      custom_op_module.ctc_beam_search_decoder_with_lm(
          ...
          lm_weight=FLAGS.lm_weight, valid_word_count_weight=FLAGS.valid_word_count_weight, #, word_count_weight = 1.00,
          top_paths=top_paths, merge_repeated=merge_repeated))

  return (
      [tf.SparseTensor(ix, val, shape) for (ix, val, shape) in zip(decoded_ixs, decoded_vals, decoded_shapes)], log_probabilities)
...
def calculate_mean_edit_distance_and_loss(model_feeder, tower, dropout, reuse):
    ...
    # batch_seq_len = tf.Print(batch_seq_len, [batch_seq_len], message="batch_seq_len: ", summarize=32)

    # Calculate the logits of the batch using BiRNN
    # logits, _ = BiRNN(batch_x, batch_seq_len, dropout, reuse)
    if('deepspeech_1' in FLAGS.model):
        logits, _, new_batch_seq_len = deepspeech_1(batch_x, tf.to_int64(batch_seq_len), dropout, reuse)
    else:
        # logits, _, new_batch_seq_len = deepspeech_2(batch_x, tf.to_int64(batch_seq_len), FLAGS.dropout_rate, reuse)

        logits, _, new_batch_seq_len = deepspeech_2(batch_x, tf.to_int64(batch_seq_len), FLAGS.dropout_rate, reuse)

        # new_batch_seq_len_shape = tf.Print(new_batch_seq_len, [new_batch_seq_len], 'new_batch_seq_len_shape: ', summarize=32)
        # debug_vars.append(new_batch_seq_len_shape)
        #
        # logits_shape = tf.shape(logits)
        # logits_shape = tf.Print(logits_shape, [logits_shape], 'logits_shape: ', summarize=32)
        # debug_vars.append(logits_shape)


        # new_batch_seq_len = batch_seq_len
        # ctc_input_length = tf.to_float(tf.multiply(batch_seq_len, tf.shape(probs)[1]))
        # new_batch_seq_len = tf.to_int32(tf.floordiv(ctc_input_length, tf.to_float(tf.shape(batch_x)[1])))
        #
        # new_batch_seq_len = compute_length_after_conv(tf.shape(batch_x)[1], tf.shape(probs)[1], batch_seq_len)
        # ctc_input_length = tf.to_float(tf.multiply(input_length, ctc_time_steps))
        # new_batch_seq_len = tf.to_int32(tf.floordiv(ctc_input_length, tf.to_float(max_time_steps)))

    if FLAGS.use_warpctc:
        total_loss = tf.contrib.warpctc.warp_ctc_loss(labels=batch_y, inputs=tf.cast(logits, tf.float32), sequence_length=new_batch_seq_len, ignore_longer_outputs_than_inputs=True)
    else:
        total_loss = tf.nn.ctc_loss(labels=batch_y, inputs=tf.cast(logits, tf.float32), sequence_length=new_batch_seq_len, ignore_longer_outputs_than_inputs=True)
    ...
    if(FLAGS.lm_decoding):
        decoded, _ = decode_with_lm(logits, new_batch_seq_len, merge_repeated=False, beam_width=FLAGS.beam_width)
    else:
        decoded, _ = tf.nn.ctc_beam_search_decoder(inputs=logits, sequence_length=new_batch_seq_len, beam_width=FLAGS.beam_width, merge_repeated=True)
    # decoc = sparse_tensor_value_to_texts(decoded[0], alphabet)
    # decoc = tf.Print(decoc, [decoc], message="Decoded_No_LM: ")

    # decoded, _ = tf.nn.ctc_beam_search_decoder(inputs=logits, sequence_length=new_batch_seq_len, beam_width=FLAGS.beam_width, merge_repeated=True)

    # decoded = tf.Print(decoded, [tf.shape(decoded[0])], 'decoded: ', summarize=32)

    # decoded_shape = np.shape(decoded)
    # print(decoded_shape)

    # Compute the edit (Levenshtein) distance
    distance = tf.edit_distance(tf.cast(decoded[0], tf.int32), truth=batch_y)
    # distance = tf.Print(distance, [distance], message="Distance: ", summarize=32)
...
def get_tower_results(model_feeder, optimizer):
    ...
    * The mean edit distance of the outcome averaged over the whole batch ``mean_edit_distance``
        ...
        for i in range(len(available_devices)):
            ...
            with tf.device(device):
                ...
                with tf.name_scope('tower_%d' % i) as scope:
                    ...
                    total_loss, avg_loss, distance, mean_edit_distance, decoded, labels = calculate_mean_edit_distance_and_loss(model_feeder, i, dropout_rates, reuse=i>0)
                    ...
                    # debugging variables
                    # tower_debug_vars.append(debug_vars)
    ...
    avg_mean_edit_ditance_towers = tf.reduce_mean(tower_mean_edit_distances, 0)

    tf.summary.scalar(name='step_loss', tensor=avg_loss_across_towers, collections=['step_summaries'])
    tf.summary.scalar(name='edit_mean_distance', tensor=avg_mean_edit_ditance_towers, collections=['step_summaries'])
...
def train(server=None, logger=None):
    r'''
    ...
    train_data = preprocess(FLAGS.train_files.split(','),
                            ...
                            n_input,
                            n_context,
                            alphabet, model = FLAGS.model,
                            hdf5_cache_path=FLAGS.train_cached_features_path, logger=logger)
    ...
    dev_data = preprocess(FLAGS.dev_files.split(','),
                          ...
                          n_context,
                          alphabet, model = FLAGS.model,
                          hdf5_cache_path=FLAGS.dev_cached_features_path, logger=logger)
    ...
    test_data = preprocess(FLAGS.test_files.split(','),
                           ...
                           n_context,
                           alphabet, model = FLAGS.model,
                           hdf5_cache_path=FLAGS.test_cached_features_path, logger=logger)
        ...
        with tf.train.MonitoredTrainingSession(master='' if server is None else server.target,
            ...
            try:
                ...
                while job and not session.should_stop():
                    ...
                    print('Loss: ' + str(job.loss) + ' | WER: ' + str(job.wer) + ' | Distance: ' + str(job.mean_edit_distance))
...
def create_inference_graph(batch_size=1, n_steps=16, use_new_decoder=False):
    ...
    logits, layers, new_seq_length = deepspeech_2(batch_x=input_tensor, seq_length=seq_length if FLAGS.use_seq_length else None,
                                                  dropout=no_dropout[0], batch_size=batch_size, n_steps=n_steps, previous_state=None, mode = 'infer')
...
def do_single_file_inference(input_folder):
        ...
        for subdir, dirs, files in os.walk(input_folder):
            for file in files:
                if (file.endswith('.wav')):
                    audio_formating(subdir + file, subdir + '/tmp/'+ file)
                    mfcc = audiofile_to_input_vector(subdir + '/tmp/'+ file, n_input, n_context)

                    with open(subdir + file.replace('.wav', '.txt'), 'r') as readTranscript:
                        phrase = readTranscript.read()
                        phrase = normalize(phrase, language='arabic')
                    ...
                    # for i in range(0, len(mfcc), FLAGS.n_steps):
                        ...
                        #     chunk = np.pad(chunk,
                        #                 calculate_mean_edit_distance_and_loss   ((0, FLAGS.n_steps-len(chunk)), (0, 0)),
                        #                    mode='constant',
                        #                    constant_values=0)
                    ...
                    decoded, _ = decode_with_lm(output, [len(output)], merge_repeated=False, beam_width=FLAGS.beam_width)
                    output = session.run(decoded[0])

                    text = sparse_tensor_value_to_texts(output, alphabet)
